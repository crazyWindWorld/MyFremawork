...
public class ProtoBatchWindowAsync : EditorWindow
{
    ...
    private async Task RunAsync()
    {
        try
        {
            ...
            string protocAbsOrEmpty = string.IsNullOrWhiteSpace(protocPath) ? "" : ResolvePath(protocPath);
    ...
    private async Task GenerateProtoCmdsAsync()
    {
        ...
        var outFile = outRoot + "/ProtoCmds.cs";
        string outputPath = ResolvePath(outFile);
        string outputAutoRegisterPath = "Assets/Scripts/Generated/Network/MessageHandlerAutoRegister.cs";
        if (File.Exists(outputPath))
        {
            File.Delete(outputPath);
        }
        Directory.CreateDirectory(Path.GetDirectoryName(outputPath));
        // 要排除的文件夹关键字（支持包含子目录匹配）
        ...
        var protoFiles = allProtoFiles
            .Where(f => !excludedFolders.Any(ex =>
                f.Replace("\\", "/").Contains($"/{ex}/"))) // 兼容不同 OS 的路径分隔符
            .ToArray();

        Debug.Log($"将生成映射表，过滤前 {allProtoFiles.Length} 个文件，过滤后 {protoFiles.Length} 个文件。");
        var sb = new StringBuilder();
        sb.AppendLine("// Auto-generated by ProtoBatchWindowAsync");
        sb.AppendLine("// DO NOT EDIT MANUALLY");
        sb.AppendLine("using System;");
        sb.AppendLine();
        sb.AppendLine("public static partial class ProtoCmds {");


        var sbAutoRegister = new StringBuilder();
        sbAutoRegister.AppendLine("// Auto-generated by ProtoBatchWindowAsync");
        sbAutoRegister.AppendLine("// DO NOT EDIT MANUALLY");
        sbAutoRegister.AppendLine("using System;");
        sbAutoRegister.AppendLine("using NetFramework;");
        sbAutoRegister.AppendLine();
        sbAutoRegister.AppendLine("public static class MessageHandlerAutoRegister");
        sbAutoRegister.AppendLine("{");
        sbAutoRegister.AppendLine("   public static void RegisterAll()");
        sbAutoRegister.AppendLine("   {");
        ...
        foreach (var file in protoFiles)
        {
            ...
            sb.AppendLine($"    // From {fileName}.proto");
            string namespaceStr = "";
            ...
            foreach (var line in lines)
            {
                ...
                if (trim.StartsWith("message "))
                {
                    ...
                    if (braceIndex >= 0)
                        cls = cls.Substring(0, braceIndex);

                    // 进一步安全过滤命名：仅保留 [A-Za-z0-9_]
                    cls = new string(cls.Where(ch => char.IsLetterOrDigit(ch) || ch == '_').ToArray());

                    int cmd = baseCmd++;
                    sb.AppendLine($"    public const ushort {cls} = {cmd};");
                    sbAutoRegister.AppendLine($"      MessageHandler.Register<{namespaceStr}.{cls}>({cmd});");
                }

            }

            baseCmd = ((baseCmd / 1000) + 1) * 1000; // 每个 proto 文件一个区间
            sb.AppendLine();
            sbAutoRegister.AppendLine();
        }
        ...
        File.WriteAllText(outputAutoRegisterPath, sbAutoRegister.ToString(), new UTF8Encoding(true));
        AssetDatabase.Refresh();
        ShowNotification(new GUIContent($"✅ Proto Cmds 已生成 → {outputPath}"));
    }
}
